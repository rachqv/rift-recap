...
- 🔊 **Sound**: an optional ambient soundtrack that plays on every page and keeps going as you move between them, a soft sound on slide changes, the archetype reveal and the champion's voice line, all behind one speaker button and a volume slider in the top bar. Off until you turn it on, and remembered in your browser (after a reload the music starts again on your first click or key press, since browsers block audio until then). Everything but the voice line is synthesized live with the Web Audio API. The champion's voice line is in the language of the page where Riot recorded one (16 of the 21 languages), and the champion that keeps beating you says their ban line on the matchups slide.
