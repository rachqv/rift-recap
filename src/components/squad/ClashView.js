 ...
 * left out when there are none. `share` is `{ cardUrl }` for the share buttons.
 */
export default function ClashView({ data, share, ranges, t = defaultT }) {
  const { clash } = data;
  const teams = {
  ...
  const slides = [
    ...
    [t("clash.rail.end"), <ClashEndSlide t={t} key="end" teams={teams} hrefs={hrefs} share={share} />],
  ].filter(Boolean);
